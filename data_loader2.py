...
class craft_base_dataset(data.Dataset):
	...
	def inference_pursedo_bboxes(self, net, image, word_bbox, word, viz=False, idx=-1,height_of_box=64.0,expand_small_box=5):
		...
		# cv2.imwrite('./vis_test/bgr_region_scores.png',bgr_region_scores)
		pursedo_bboxes = watershed(input, bgr_region_scores, idx=idx, viz=False)
		# for i in range(pursedo_bboxes.shape[0]):
		# 	cv2.imwrite('./vis_test/pursedo_bboxes_'+str(i)+'.png',pursedo_bboxes[i])
		_tmp = []
		for i in range(pursedo_bboxes.shape[0]):
		...
		try:
			for j in range(len(bboxes)):
				...
				# cv2.imwrite('./vis_test/'+str(j)+'.png',input[int(bb[0,1]*scale):int(bb[2,1]*scale),int(bb[0,0]*scale):int(bb[1,0]*scale)])
...
class WordLevelDataset(craft_base_dataset):
	...
	def process_multi_folder(self, list_folders):
		...
		for folder in list_folders:
			...
			for img_file in img_files:
				...
				gt_fname = "{}.txt".format(img_name_without_ext)
	...
	def load_image_gt_and_confidencemask(self, index):
		...
		gt_path = os.path.join(gt_folder, "%s.txt" % os.path.splitext(file_name)[0])
