...
class craft_base_dataset(data.Dataset):
	...
	# def inference_pursedo_bboxes(self, net, image, word_bbox, word, viz=False, idx=-1,height_of_box=64.0,expand_small_box=5):
	...
	# 	pursedo_bboxes_1 = bboxes.copy().astype(int)
	# 	# print(pursedo_bboxes.shape)
	# 	# thickness = 4
	# 	# for i in range(pursedo_bboxes_1.shape[0]):
	# 	# 	bbox = pursedo_bboxes_1[i,:, :]
	# 	# 	bbox = np.expand_dims(bbox, axis = 0)
	# 	# 	# print(bbox.shape)
	# 	# 	bbox = bbox.reshape((-1,1,2))
	# 	# 	vis_img = cv2.polylines(vis_img,[bbox],False,(0,255,255), thickness= thickness)
	# 	# cv2.imwrite('./vis/'+str(datetime.now())+'.png', vis_img)
	...
	def inference_pursedo_bboxes(self, net, image, word_bbox, word, viz=False, idx=-1,height_of_box=64.0,expand_small_box=5):
		...
		bgr_region_scores = cv2.cvtColor(bgr_region_scores, cv2.COLOR_GRAY2BGR)
		pursedo_bboxes = watershed(input, bgr_region_scores, idx=idx, viz=False)
		# cv2.imwrite('./test/input_{}.jpg'.format(str(datetime.now())),input)
		_tmp = []
		for i in range(pursedo_bboxes.shape[0]):
			if np.mean(pursedo_bboxes[i].ravel()) > 2:
				_tmp.append(pursedo_bboxes[i])
		...
		height = input.shape[0]
		if confidence <= 0.5:
			# roi_gray = cv2.cvtColor(input,cv2.COLOR_BGR2GRAY)
			...
			#   	areas_ind[area] = ind
			#   	areas.append(area)
			# areas = sorted(areas,reverse=True)[1:]
			# if len(areas) == len(word):
			# 	vis_img = input.copy()
			# 	for i in areas:
			...
			# 	cv2.imwrite(word+'.png', vis_img)

			# else:
			set_3 = set('aAàÀảẢãÃáÁạẠăĂằẰẳẲẵẴắẮặẶâÂầẦẩẨẫẪấẤậẬbBcCdDđĐeEèÈẻẺẽẼéÉẹẸêÊềỀểỂễỄếẾệỆfFgGhHJkKlLnNoOòÒỏỎõÕóÓọỌôÔồỒổỔỗỖốỐộỘơƠờỜởỞỡỠớỚợỢpPqQrRsStTuUùÙủỦũŨúÚụỤưƯừỪửỬữỮứỨựỰvVxXyYỳỲỷỶỹỸýÝỵỴzZ0123456789')
			set_4 = set('wmWM')
			set_1 = set('iIìÌỉỈĩĨíÍịỊ')
			count = 0
			for char in word:
				if char in set_3:
					count += 3
			...
			for i, char in enumerate(word):
				top_left = (int(old_bottom_right[0]), 0)
				if char in set_3:
					bottom_right = (int(top_left[0]+ele*3), height)
				elif char in set_4:
					bottom_right = (int(top_left[0]+ele*4), height)
				else:
					bottom_right = (int(top_left[0]+ele), height)
				old_bottom_right = bottom_right
				bbox = np.array([[top_left[0], top_left[1]], [bottom_right[0], top_left[1]], [bottom_right[0], bottom_right[1]], [top_left[0], bottom_right[1]]])
				bboxes.append(bbox)
			# width_per_char = width / len(word)
			# for i, char in enumerate(word):
			# 	if char == ' ':
			...
			# print("vis_img")
			# im_id = str(datetime.now())
			# vis_img = input.copy()
			# vis_res = input.copy()
			# vis_dau = input.copy()
			# vis_final = input.copy()
			max_std = 0
			# vis_img = input.copy()
			residual_boxes = []
			bboxes = pursedo_bboxes
			...
			for i in range(bboxes.shape[0]):
				box =  bboxes[i,:,:]
				residual_box = input[0:int(box[1,1]), int(box[0,0]):int(box[1,0])]
				# cv2.rectangle(vis_res,(box[0, 0], 0),(box[1, 0], box[1, 1]),(0,255,0),2)
				# print(residual_box.std()) 
				if 60 < residual_box.std():
					check_dau = True
					bboxes[i, 0, 0] = box[0, 0]
					bboxes[i, 0, 1] = 0
					bboxes[i, 1, 0] = box[2, 0]
					bboxes[i, 1, 1] = 0
					# residual_box_coor = np.array([[box[0, 0], 0], [box[1,0], 0], [box[1, 0], box[1, 1]], [box[0, 0], box[0, 1]]])
					# cv2.rectangle(vis_dau,(box[0, 0], 0),(box[1, 0], box[1, 1]),(0,255,0),2)
					# cv2.rectangle(vis_final, (box[0, 0], 0), (box[2,0], box[2,1]), (0,255,0),2)
					# residual_boxes.append(residual_box)
			# if check_dau :
			# 	cv2.imwrite('./vis_#/' + im_id + '_1.png', vis_img)
			# 	cv2.imwrite('./vis_#/' +im_id + '_2.png', vis_res)
			# 	cv2.imwrite('./vis_#/'+ im_id +'_3.png', vis_dau)
			# 	cv2.imwrite('./vis_#/'+ im_id + '_4.png', vis_final)
...
class CharLevelDataset(craft_base_dataset):
	def __init__(self,icdar2013_folder, target_size=768, region_threshold=0.3,
	             affinity_threshold=0.15, viz=False, debug=False):
		super(CharLevelDataset, self).__init__(target_size, region_threshold, affinity_threshold, viz, debug)
		...
		if len(word_bboxes) > 0:
			...
			for i in range(len(word_bboxes)):
				...
				pursedo_bboxes, bbox_region_scores, confidence = self.inference_pursedo_bboxes(self.net, image,
				                                                                               word_bboxes[i],
				                                                                               words[i],
				                                                                               viz=self.viz, idx=i,height_of_box=height_of_box,expand_small_box=expand_small_box)
